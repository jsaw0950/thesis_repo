...
class Individual:
    ...
    def evaluate(self, fitness_func, X_train, X_test, y_train, y_test):
        self.fitness, self.best_model = fitness_func(self.genes, X_train, X_test, y_train, y_test)
    ...
    def optimize(self):
        ...
        for i in range(self.max_generations):
            ...
            for j in range(self.num_individuals):
                ...
                if self.population[j].fitness > self.best_fitness or self.best_fitness == -1:
                    self.best_individual = self.population[j]
                    self.best_fitness = self.population[j].fitness
            ...
            print(self.best_individual.fitness, time.time() - start)
            self.fitness_arr.append(self.best_individual.fitness)
        ...
        return (self.best_individual.genes, self.best_fitness, self.fitness_arr, self.best_model)
...
def main():
    ...
    optimizer = GeneticAlgorithm(svm_eval, init_pos, num_individuals=10, bounds=[(0.01, 10), (0.01, 10)], max_generations=100, mutation_rate=1.0)
